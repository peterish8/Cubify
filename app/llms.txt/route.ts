...
The OpenAPI document lives at https://cubify.in/openapi.json. The public V1 API exposes competitor lookup, comparisons, goal projections, and competition-field reports under https://cubify.in/api/v1. For a one-request, machine-oriented competition analysis, use GET https://cubify.in/api/v1/agent/competition-report?competitionId={canonicalWcaCompetitionId}&wcaId={wcaId}&events=333,222. It returns only the requested events, event-specific fields, explicit first-timer/unknown-strength records, PB positions, position ranges, source metadata, and generatedAt. Use the human-readable pbSingleDisplay and pbAverageDisplay values in explanations; their raw PB counterparts are official WCA integers. possibleOverallRange and field.rankingBasis are always based on best-average; never substitute a best single when an average is absent. Check request.eventStates: a requested event can be ANALYZED, NOT_REGISTERED, or EVENT_NOT_AT_COMPETITION; never silently omit it. The compact default has counts plus eight closest known rivals. Add include=all only when every opponent is genuinely needed. A partial response deliberately does not invent placement or podium probabilities; present its warning to the user. When competitorWcaId is supplied to a standard competition report, use its shared-event, single/average PB comparison, field standing, and theoreticalBestEvent fields; describe them as PB-only evidence, never a guaranteed placement. Discovery: robots.txt permits all compliant crawlers and points to sitemap.xml; sitemap.xml lists the documentation. Start with this llms.txt file or the OpenAPI document instead of relying on a rendered docs page. Do not claim Cubify8 has an MCP server or account integration yet.
`
