...
export function GET() {
  return NextResponse.json({
    ...
    paths: {
      ...
      "/api/v1/agent/competition-report": { get: { summary: "Get a compact competition report for an AI agent", description: "Cubify fetches official WCA registrations, competitor records, recent form, and round metadata server-side. Clients should call this endpoint directly instead of scraping or searching WCA pages.", parameters: [{ name: "competitionId", in: "query", required: true, schema: { type: "string", example: "CubeathonBengaluru2026" } }, { name: "wcaId", in: "query", required: true, schema: wcaId }, { name: "events", in: "query", required: false, description: "Comma-separated WCA event IDs. Defaults to the competitor's registered events.", schema: { type: "string", example: "333,222,444" } }, { name: "include", in: "query", required: false, description: "Use all to include every opponent. The default returns only the closest known rivals.", schema: { type: "string", enum: ["all"] } }, { name: "offset", in: "query", required: false, description: "Zero-based opponent offset when include=all.", schema: { type: "integer", minimum: 0, maximum: 5000, default: 0 } }, { name: "limit", in: "query", required: false, description: "Number of opponents when include=all (maximum 100).", schema: { type: "integer", minimum: 0, maximum: 100, default: 100 } }], responses: { ...success, "502": { description: "WCA response was invalid or unavailable" } } } },
    },
  })
}
